...
#setwd()
setwd("/Users/elanus/Library/CloudStorage/Dropbox/_2024_SexSelSpeciation/07_Data/Export/GitHub/v02") # iMac-Home
...
Figure_Meta_by_Year <- plot_grid(Plot_SampleSize_by_Year, Plot_ES_by_Year,
                                 ...
                                 nrow = 2)
Figure_Meta_by_Year
ggsave("Figure_1.pdf", width = 7, height = 10, device = cairo_pdf)
...
ggsave("Figure_2.pdf", width=6.5, height=8.5, device = cairo_pdf)
...
ggsave("Figure_3.pdf", width=6, height=9, device = cairo_pdf)
...
Phylo_Tree <- rotate(p_nodes, 55) %>% flip(80, 59) %>% flip(61, 62)
Phylo_Tree

Phylo <- ggtree(Tree_pruned_ALL, size = 0.7) +
  #ylim(-2, 51) +
  xlim(-80, 650) +
  annotate("rect", xmin = 380, xmax = 520, ymin = 54.5, ymax = 53.5, fill = palette_1[8], alpha = .8) + # Animalia
  annotate("rect", xmin = 380, xmax = 520, ymin = 53.5, ymax = 52.5, fill = palette_1[1], alpha = .8) + # Araneae
  annotate("rect", xmin = 380, xmax = 520, ymin = 52.5, ymax = 45.5, fill = palette_1[2], alpha = .8) + # Insecta
  annotate("rect", xmin = 380, xmax = 520, ymin = 45.5, ymax = 44.5, fill = palette_1[8], alpha = .8) + # Vertebrata
  annotate("rect", xmin = 380, xmax = 520, ymin = 44.5, ymax = 22.5, fill = palette_1[3], alpha = .8) + # Fishes
  annotate("rect", xmin = 380, xmax = 520, ymin = 22.5, ymax = 19.5, fill = palette_1[4], alpha = .8) + # Amphibia
  annotate("rect", xmin = 380, xmax = 520, ymin = 19.5, ymax = 13.5, fill = palette_1[5], alpha = .8) + # Reptilia
  annotate("rect", xmin = 380, xmax = 520, ymin = 13.5, ymax =  2.5, fill = palette_1[6], alpha = .8) + # Aves
  annotate("rect", xmin = 380, xmax = 520, ymin =  2.5, ymax =  0.4, fill = palette_1[7], alpha = .8) + # Mammalia
  geom_tiplab(size = 3.4, color = "black", hjust = 0.0) +
  add_phylopic(img = icon_insect,    x = 600, y = 49, height = 2.5,  alpha = 0.8, fill = "black") +
  add_phylopic(img = icon_fish,      x = 600, y = 34, height = 1.8,  alpha = 0.8, fill = "black") +
  add_phylopic(img = icon_amphibian, x = 600, y = 21, height = 3.2,  alpha = 0.8, fill = "black") +
  add_phylopic(img = icon_reptile,   x = 600, y = 15, height = 5.0,    alpha = 0.8, fill = "black") +  
  add_phylopic(img = icon_bird,      x = 600, y = 9,  height = 3.5,  alpha = 0.8, fill = "black") +
  add_phylopic(img = icon_mammal,    x = 600, y = 2,  height = 3.5,    alpha = 0.8, fill = "black") +
  theme_tree()
Phylo

Phylo <- rotate(Phylo, 55) %>% flip(80, 59) %>% flip(61, 62)
Phylo
...
ggsave("Figure_S2.pdf", width=8, height=10, device = cairo_pdf)
...
ggsave("Figure_S3.pdf", width = 8, height = 10, device = cairo_pdf)
...
OrchaRD_Plot_Class <- orchaRd::orchard_plot(REML_r_Mod_Class, 
  ...
  theme(panel.border = element_blank(), 
        ...
        plot.margin = margin(t =  25,  # Top margin
                             ...
                             l =  1)) # Left margin 
OrchaRD_Plot_Class
ggsave("Figure_S4.pdf", width=5.5, height=7.5, device = cairo_pdf)
...
ggsave("Figure_S5.pdf", width=9, height=8, device = cairo_pdf)


##  * Figure S6 ####

Data$z_SE <- sqrt(Data$z_Var) # standard error

REML_Model_All_Null_phylo_PubBias_z <- rma.mv(z, z_Var,
                                              mod =~z_SE + Year + PhyloCorrection,
                                              random=list(~ 1 | Taxon_Phylo, # phylo effect
                                                          ~ 1 | Taxon, # non-phylo effect
                                                          ~ 1 | Study_ID, 
                                                          ~ 1 | Obs_ID), 
...
REML_Model_All_Null_phylo_PubBias_z_except_SE <- rma.mv(z, z_Var,
                                                        mod =~Year + PhyloCorrection,
                                                        random=list(~ 1 | Taxon_Phylo, # phylo effect
                                                                    ~ 1 | Taxon, # non-phylo effect
                                                                    ~ 1 | Study_ID, 
                                                                    ~ 1 | Obs_ID), 
...
res_SE <- resid(REML_Model_All_Null_phylo_PubBias_z_except_SE)

REML_Model_All_Null_phylo_PubBias_z_SE <- rma.mv(res_SE, z_Var,
                                                  mod =~z_SE,
                                                  random=list(~ 1 | Taxon_Phylo, # phylo effect
                                                              ~ 1 | Taxon, # non-phylo effect
                                                              ~ 1 | Study_ID, 
                                                  ...
                                                  test="t", # using t dist rather than z 
                                                  data=Data)
REML_Model_All_Null_phylo_PubBias_z_SE_intercept<-(coef(summary(REML_Model_All_Null_phylo_PubBias_z_SE)))$estimate[1]
REML_Model_All_Null_phylo_PubBias_z_SE_slope<-(coef(summary(REML_Model_All_Null_phylo_PubBias_z_SE)))$estimate[2]


Scatter_z_versus_z_SE <- ggplot(data = Data,
                                mapping = aes(x = z_SE, y = res_SE)) +
  labs(title = "", x = "Standard Error", y = expression("Residual Fisher's"~italic(z))) +
  scale_y_continuous(limits = c(-1.3, 0.6), breaks = c(-1.2, -0.6, 0, 0.6)) + 
  scale_x_continuous(limits = c(0, 0.75), breaks = c(0, 0.2, 0.4, 0.6, 0.8)) + 
  geom_abline(slope=REML_Model_All_Null_phylo_PubBias_z_SE_slope,intercept = REML_Model_All_Null_phylo_PubBias_z_SE_intercept, size=0.8, color = "#08519C", linetype = "longdash") +
...
REML_Model_All_Null_phylo_PubBias_z_except_Year <- rma.mv(z, z_Var,
                                                          mod =~z_SE + PhyloCorrection,
                                                          random=list(~ 1 | Taxon_Phylo, # phylo effect
                                                                      ~ 1 | Taxon, # non-phylo effect
                                                                      ~ 1 | Study_ID, 
                                                                      ~ 1 | Obs_ID), 
...
res_year <- resid(REML_Model_All_Null_phylo_PubBias_z_except_Year)

REML_Model_All_Null_phylo_PubBias_z_Year <- rma.mv(res_year, z_Var,
                                                   mod =~Year,
                                                   random=list(~ 1 | Taxon_Phylo, # phylo effect
                                                               ~ 1 | Taxon, # non-phylo effect
                                                               ~ 1 | Study_ID, 
...
REML_Model_All_Null_phylo_PubBias_z_Year_intercept<-(coef(summary(REML_Model_All_Null_phylo_PubBias_z_Year)))$estimate[1]
REML_Model_All_Null_phylo_PubBias_z_Year_slope<-(coef(summary(REML_Model_All_Null_phylo_PubBias_z_Year)))$estimate[2]

Scatter_z_versus_year <- ggplot(data = Data,
                                mapping = aes(x = Year, y = res_year)) +
  labs(title = "", x = "Year of publication", y = expression("Residual Fisher's"~italic(z))) +
  scale_y_continuous(limits = c(-1.4, 0.6), breaks = c(-1.2, -0.6, 0, 0.6)) + 
  scale_x_continuous(limits = c(1995, 2025), breaks = c(1995, 2000, 2005, 2010, 2015, 2020, 2025)) + 
  geom_abline(slope=REML_Model_All_Null_phylo_PubBias_z_Year_slope,intercept = REML_Model_All_Null_phylo_PubBias_z_Year_intercept, size=0.8, color = "#08519C", linetype = "longdash") +
  geom_point(shape = 21, size = 5, fill = "grey", alpha = .6) +
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
...
ggsave("Figure_S6.pdf", width=6, height=9, device = cairo_pdf)
